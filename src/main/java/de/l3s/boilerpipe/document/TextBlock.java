...
public class TextBlock implements Cloneable {
	...
	public Attributes getTagAttrs() {
		return tagAttrs;
	}

	public void setTagAttrs(Attributes tagAttrs) {
	...
	public void mergeNext(final TextBlock other) {
		...
		if (containedTextElements == null) {
			containedTextElements = (BitSet) other.containedTextElements
					.clone();
		} else {
	...
	private void initDensities() {
		...
		linkDensity = numWords == 0 ? 0 : numWordsInAnchorText
				/ (float) numWords;
	}
	...
	public String toString() {
		return "[" + offsetBlocksStart + "-" + offsetBlocksEnd + ";tl="
				+ tagLevel + "; nw=" + numWords + ";nwl=" + numWrappedLines
				+ ";ld=" + linkDensity + "]\t"
	...
	/**
	 * Adds a set of labels to this {@link TextBlock}. <code>null</code>
	...
	protected TextBlock clone() {
		...
		if (containedTextElements != null) {
			clone.containedTextElements = (BitSet) containedTextElements
					.clone();
		}
