...
public class BoilerpipeHTMLContentHandler implements ContentHandler {
	...
	public void flushBlock(String localName) {
		if (inBody == 0) {
		...
		if (tb.getTagName() == null) {
			tb.setTagName(localName);
		}
	...
	public TextDocument toTextDocument() {
		// just to be sure
		flushBlock();
		TextDocument td = new TextDocument(getTitle(), getTextBlocks());
		// Set the extra attributes.
		td.setImage(getImage());
		td.setDescription(getDescription());
		td.setFavicon(getFavicon());
		td.setKeywords(getKeywords());
		td.setAuthors(getAuthors());
		return td;
	}
